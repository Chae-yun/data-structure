...
int main(void){
	...
	//숫자가 많아지면 퀵정렬은 정렬될 수록 안좋으니까 프로그램이 죽어버린다
	//계속 피봇은 왼쪽이고 재귀호출 되니까 오른쪽은 계속계속 스택에 쌓인다
	//그 층층이 쌓는 것이 문제 10000개나 쌓을 수 없대
	//왼쪽은 종료되지만 오른쪽은 종료가 안돼
	//재귀호출하면서 종료도 많이 되면 괜찮은데 종료가 안되고 계속 쌓이면 문제인 것이야
	//기본 스택은 1메가라서 스택에 층을 많이 못 쌓아
	//환경설정에서 고칠 수는 있다
