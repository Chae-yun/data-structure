...
int main(void){
	...
	for (i = 0; i < SIZE; i++) if (departureVertex == V[i]) break;
	D[i] = 0;
	...
	for (i = 0; i < SIZE; i++){
		if (V[i] == departureVertex) continue;
		printf("%c", V[i]);
		vertexNum = i;
		while (V[vertexNum] != departureVertex){
